...
public class UserProfile : Profile
{
    public UserProfile()
    {
        CreateMap<User, UserDto>()
            .ForMember(dest => dest.Subjects, opt => opt.MapFrom((src, _, __, context) =>
            {
                // If student, get subjects from enrollments
                if (src.Role?.Name == "Student")
                    return src.Enrollments?.Where(e => e.Subject != null).Select(e => context.Mapper.Map<SubjectDto>(e.Subject)).ToList() ?? new List<SubjectDto>();
